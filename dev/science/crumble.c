...
int check_crumble_param_validity_and_consistency(dataptr dz)
{
	...
	double processtime, srate = (double)dz->infile->srate, frqratio, maxrandlenincr, maxrandlenshrnk;
	...
	maxrandlenincr  = 1.5 * dz->param[CRRAND];							//	1.5 is max random length increase
	maxrandlenshrnk = 0.5 * dz->param[CRRAND];							//	0.5 is max random length decrease
...
int create_crumble_sndbufs(dataptr dz)
{
	...
	dz->sampbuf[5] = dz->sampbuf[4] + dz->maxinseg;			
	return(FINISHED);
}
